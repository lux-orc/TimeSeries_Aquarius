...
create or replace table h_rain as
    select
        ...
        #2 as Rain
    from '_h2d_test/data/hRain_wide.parquet'
;
    ...
    with tmp as (
        select
            *,
            ...
            time_bucket('1 day', Time - '1 hour'::interval)::date as "Date"
        from h_rain
    )
    select
        "Date",
...
-- copy d_rain_agg to '_h2d_test/daily_rain_duckdb.csv';


-- Read the offset daily rainfall time series and fix it
create or replace table d_rain as
    select * replace((strptime(Date, '%Y-%m-%d') - '1 day'::interval)::date as Date)
    from '_h2d_test/data/dRain_wide.parquet'
;
